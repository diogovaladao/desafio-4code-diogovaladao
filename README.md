# Desafio 4Code
Bem vindo ao Desafio 4Code. Você pode fazer este teste da seguinte maneira:
...
- A API deve ser em Node (com typescript) ou  C# com .NET Core.
- Não é necessário utilizar banco de dados, caso use, configure um banco de dados em memória (h2 por exemplo).
...
**Premissas:**
- O Projeto deve ser feito em React utilizando Typescript.
- A API que pode ser acessada no link `sem link por enquanto`.
- A lista de aeroportos pode ser obtida com um GET da URL `sem link por enquanto`.
- A lista de voos por dia é um POST na `sem link até o momento`, com o Body seguindo este padrão em Json:
```json
{
	"from": "BSB",
	"to": "VCP",
